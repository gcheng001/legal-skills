---
name: case-os
description: '案件操作系统总控。统一入口和调度器，加载全局规则，调度独立Skill。TRIGGER when: 用户输入包含"案件OS"、"案件 OS"、"case-os"、"case os"（不区分大小写）。'
metadata:
...
# case-os 案件操作系统（总控）v10.0

## 工作定位

`case-os` 是案件操作系统的**统一入口和调度器**，承担三件事：
...
1. 案件OS可在当前调用端独立完整执行；Codex 与 Claude Code 是两个可互换入口，不要求从一端切换到另一端。
...
### Phase A（材料处理）— 用户操作 A1-A5 + 内部 A6 门禁

> **核心目标**：快速整理材料 + 快速了解基本案情。深度分析留给 Phase B 九步法展开。

| 步骤 | 独立Skill | 职责 | 前置条件 | 必须确认 |
|------|----------|------|----------|----------|
| A1 | `case-git-init` | 案件初始化（Git + 目录结构 + 核心文件） | 无 | 否 |
| A2 | `case-ocr` | 材料扫描+OCR转换（Mineru优先） | A1完成 | 否（视频截图需人工审阅） |
| A3 | `case-archive` | 文件夹自动归档 | A2完成 | 否（事后汇报） |
| A4 | `case-info-extract` | 案件理解（信息提取 + 案情概要，一次确认） | A3完成 | **是**（唯一确认点） |
| A5 | `case-evidence-cards` | 证据卡片与关系复核（双视图） | A4确认 | **是**（律师复核） |
| A6（内部） | `manage_integration_state.py` | 本地状态汇总与发布准备 | A5复核后自动触发 | 否（不得作为单独人工步骤） |

**Phase A红线**：用户操作步骤必须按 A1→A2→A3→A4→A5 顺序依次执行。A5 明确复核后，状态生成器须在同一后置动作内自动完成 A6 内部门禁；不得停顿并提示律师“执行 A6”。每个用户操作步骤完成后 git 提交。

**Phase A→B切换**：A5 复核后自动通过 A6 门禁时，输出切换提示：
```
✅ Phase A 全部完成（6步）。
📌 下一步：请切换到 GLM 5.1 模型执行 Phase B（九步法分析）。
执行命令：cc-switch-to.sh glm
然后重新启动 Claude Code 会话，输入"案件 OS"继续。
```
...
|------|----------|------|----------|------------|----------|
| S1 | `case-s1-fixed-claim` | 固定权利请求 | A5复核后内部A6自动通过 | — | **是** |
...
| S10 | `case-s10-hallucination` | 法条幻觉校验+八个一致 | S1-S9有法条引用时触发 | — | S10守门员阻断 |
...
**S10触发条件**：扫描intermediate/原告九步法/下所有.md文件，检测法条引用模式，有引用才触发。
...
| 规范文本生成 | `legal-doc-drafter` | 九步法完成后，按需调用（起诉状、代理词、答辩状等） |
| 庭审准备 | `prepare-hearing-record` | S10通过后，律师确认 |
...
**判断依据**：
1. **版本号检查**：CLAUDE.md 包含 `*本文件由案件OS v10.0生成*` 字样
...
案件OS使用Hook机制自动执行后置脚本，确保每个Skill完成后：
...
- 对网页、案件仪表盘、可视化视图、可视化 Tab、客户汇报页、封面图、说明图、图标、图例、空状态图等非证据视觉资产，优先使用 Claude Code/可用图像工具当前可用的能力完成。
- 可使用的能力包括但不限于：ChatGPT Image 2、Claude Code 可用图像生成能力、前端代码生成、本地预览与截图验证能力。
...
- v10.0-claude-20260526-a6-internal - **A6 内部门禁纠偏**：A5 明确确认后自动完成 A6 状态校验并消解被新确认覆盖的迁移冲突；A6 不再作为用户待执行步骤暴露。
- v10.0-claude-20260526-local-contract - **跨端本地契约同步**：同步双文件结构化状态、纯本地 hook 与 A1-A6 唯一入口口径；legacy 页面和 `agents/` 外联资产不进入正常流程。
...
- v10.0-codex-20260519 - **历史上游记录（未启用）**：Codex 源侧曾合入 `agents/` 子系统；Claude 端本次不把该目录纳入正常流程，短信监控、飞书同步和 LaunchAgent 仍为显式 opt-in。

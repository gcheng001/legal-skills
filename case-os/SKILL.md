...
# civil-case-os 民事案件操作系统（总控）v12.0
...
## 轻量路总则（ADR-0005，2026-09-15）

**档位开关**：默认 `light`。新案一律轻量路；续办时未声明「升级 / 全链 / 双视图 / 开庭前」也走轻量路；进行中的 full 旧案保持 full。轻量路底座是九步中间层：请求权基础、要件拆解、举证责任、事实认定、S10 幻觉校验全部保留。

**三门停**：全链只有三个停点——①A4 当事人与请求确认；②S1-S10 连续跑完后的九步整包确认（含自动 S10）；③对外文书生成前确认。A5 飞书、A6 证据卡片照做但不单独拦主链，与九步包一起看。九步包未确认前，不得生成起诉状/答辩状；S10 阻断时不得进 FINAL。

**总控直跑**：轻量路下关闭「动脑活必须派师傅才能写盘」硬闸，总控直接执行；检索/文书/质检按需借调 SuitAgent，不自动派。单视图：只跑代理这一侧九步目录，S3/S6 仍读对方起诉状或答辩状做抗辩和争点，不另建对侧全套九步。A→B 不切模型不换会话；Git 在九步包结束后一次提交。

**底线不变**：文件不得擅自删改；S10 法条幻觉自动阻断进 FINAL；编造事实、跳过三门直接出文书仍是 Hard Fail。变更用「重跑受影响下游」替代整链重来。

## 专人派发体制（ADR-0004，2026-08-19；full 档启用，light 档休眠）
...
**报告整合（Reporter，2026-08-19 复议采纳；落盘 2026-09-15 按四区制修订）**：S10通过后/开庭前/结案时派 Reporter 师傅整合全链产物出**案件分析报告**（深研报告）、庭前报告、结案报告；律师点名要报告随时派。产出落盘：深研报告属过程件写 `分析材料/`；庭前报告、结案报告、对客 Word 报告属**成品**写 `交付/`。属整合件不改动各步结论。落盘规范统一见 SuitAgent `references/rules/CaseOsLayout.md` v2.0（用途四区制：成品→交付；原件→材料夹；过程→分析材料；机器件→_archive）。

**派发硬闸（2026-08-19 验证缺陷修复）**：仅 full 档生效。动脑活产物（A4确认表、证据卡片、九步法各步、质证意见、起诉状/答辩状、庭前策略手册等）**未经师傅派发，总控不得直接写入案件目录**。唯一豁免：subagent 工具不可用或律师明示跳过——此时必须在 LOG.md 记录「未派发原因」一行，否则视为绕闸（Hard Fail）。机器活（核对表/状态同步/LOG/CLAUDE.md）不受此闸约束。light 档按上文轻量路总则由总控直跑，本闸休眠。
...
- **确认机制红线**：light 档三门停（A4 → 九步整包 → 对外文书），S2/S4/S7 北大法宝复验不豁免；full 档 A4/S1/S5/S6/S8/S9 必须律师确认
- **Git管理铁律**：light 档九步包结束后一次提交；full 档每个 Phase A 步骤后自动提交、Phase B 及之后每次修改后手动提交
...
- `CLAUDE.md` → `LOG.md` → `_archive/case-os-state.json` → `intermediate/_index.json` → 当前待办步骤已生成产物 → 档位（`CLAUDE.md` 的档位行；无记录默认 light，full 旧案保持 full）
3. 续办前输出简短断点摘要；不得擅自重做已经完成或待律师确认的步骤

### 双模型会话工作流（仅 full 档；light 档不切模型不换会话）
...
| **庭前三宝流水线** | `mqc-chronicle-master` → `mqc-legal-relation-master` → `mqc-trial-confrontation-master` | 律师要"庭前三件套"时按序串联出底稿+两图，规则见下方【庭前三宝流水线】 |
| 法院短信 | `wenshu-download` | 收到法院短信（原 case-court-sms 职能已并入 v3.3） |
| 案件讨论 | `case-discussion` | 随时发起（召见 Strategist 时产物标【立场参考】） |
| 定期扫描 | `case-scan` | 每周自动/手动触发 |
| 经验卡 | `case-experience-card` | S9完成/开庭后/判决后 |

### 庭前三宝流水线（2026-09-26 增补；源：mqc-nlv 仓库设计思想）

律师要"庭前三件套"（时间轴底稿/关系图/对抗图）时，按序串联，不并行、不倒序：

1. **事实底稿**：先跑 `mqc-chronicle-master`，产出 `casefile.json` 事实底稿——校验全绿才算底稿成立；缺材料的缺口当场留痕（标"缺口待补"），不得编造补位
2. **关系图**：把同一份底稿喂 `mqc-legal-relation-master`，出关系图
3. **对抗图**：再喂 `mqc-trial-confrontation-master`，出对抗图
4. **编号对账**：三图出齐后跑编号对账——同一事件/主体在三图中的 fid 必须一致；不一致以底稿为唯一真源回改下游图
5. **缺口留痕**：最终对账表列出全部缺口及其来源环节，随三图一并交付

约束：

- 用户只要两件（如只要底稿+关系图）时允许跳步串联，但对账表必须注明缺失环节"无底稿来源"
- V5（判断类字段）必须带三标（依据/出处/置信度），V3/V4（照录类字段）纯照录原文——编排只负责串联与对账，不越权代判断

### 条件章节重编号（2026-09-26 增补；源：龚家勇案件分析报告模板设计）

检索型报告（案件分析报告、Word 研判报告等含"检索/核查"环节的报告）的章节规则：

- **空节整节省略**：某节经检索/核查后确无内容（如无类案、无相反证据、无特殊程序事项），整节省略，禁止写"本节暂无内容/有待补充"之类占位句
- **序号前移**：省略空节后，目录与正文章节序号连续前移重排，不留跳号
- **DOCX 转换同步**：转 Word 时按重排后的序号生成目录与标题，目录必须与正文一致
- **两套编号勿混**：工作流步骤号（A1-A7、S1-S10）≠ 报告章节号；报告章节按实际保留的节重新编号，严禁把工作流步骤号当章节号写进报告标题或目录
...
| `/法院短信` | wenshu-download | 下载文书＋期限计算＋双通道提醒（原 case-court-sms 职能已并入） |
...
- `~/.local/bin/cc-switch-to.sh` — 模型切换脚本（full 档用；light 档 A→B 直跑不切换）
...
## 版本历史
- v12.1 - **庭前三宝流水线 + 条件章节重编号（2026-09-26）**：①"Phase B之后（事件驱动）"新增庭前三宝调度行——要"庭前三件套"时按 mqc-chronicle-master（出 casefile.json 事实底稿，校验全绿）→ mqc-legal-relation-master（同底稿出关系图）→ mqc-trial-confrontation-master（出对抗图）串联，三图跑 fid 编号对账（同一事件在三图一致）、缺材料缺口留痕；用户只要两件允许跳步但对账表注明"无底稿来源"；V5 判断带三标、V3/V4 纯照录，编排不越权代判断②新增条件章节重编号条款——检索型报告空节整节省略、不写占位句、目录正文序号前移、DOCX 转换同步；工作流步骤号≠报告章节号（防混两套编号）

- v12.0 - **默认轻量路（ADR-0005）**：新案默认 light，三门停（A4 → 九步整包 → 对外文书），总控直跑不自动派师傅，单视图，A→B 不切模型不换会话，九步包结束一次提交；full 档保留 ADR-0004 全部机制。改调度不重写九步正文，各 case-s* 新增 SKILL.light.md 轻量路说明。

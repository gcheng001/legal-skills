...
> 默认档位为 light（ADR-0005 轻量路，见下文「轻量路总则」）；本表逐步确认与双视图为 full 档机制。
...
| A5 | `case-feishu-sync` | 飞书同步（CLAUDE.md→云文档 + 案件池更新） | A4确认 | light 否（后台执行，结果随九步包汇报）；full **是**（同步结果确认） |
| A6 | `case-evidence-cards` | 证据卡片与关系复核（双视图） | A5后自动 | light 否（卡片照做，随九步包一起看）；full **是**（律师复核） |
...
A6 复核后自动通过 A7 门禁时，输出切换提示（light 档）：
```
✅ Phase A 完成，A4 已确认。
📌 轻量路直跑：不切模型、不换会话，现在连续执行 S1-S10（S10 自动校验），跑完后九步整包一次性确认。
📌 说「升级 / 全链 / 双视图 / 开庭前」可切 full 档（逐步确认 + 派师傅 + 双视图 + 切模型）。
```
...
|------|----------|------|----------|------------|----------|
| S1 | `case-s1-fixed-claim` | 固定权利请求 | A6复核后内部A7自动通过 | — | light 否（整包确认）；full **是** |
| S2 | `case-s2-claim-basis` | 请求权基础 | S1完成 | ✅ 必须 | light 否（整包确认）；full **是**（终选） |
| S3 | `case-s3-defense` | 抗辩规范 | S2完成 | ✅ 必须 | light 否（整包确认）；full **是**（战术） |
| S4 | `case-s4-elements` | 要件拆解 | S3完成 | ✅ 必须 | light 否（整包确认）；full **是** |
| S5 | `case-s5-case-search` | 主张检索 | S4完成 | — | light 否（整包确认）；full **是** |
| S6 | `case-s6-dispute-matrix` | 争点矩阵（双方共用） | S5完成 | — | light 否（整包确认）；full **是** |
| S7 | `case-s7-burden` | 举证责任分配 | S6完成 | ✅ 必须 | light 否（整包确认）；full **是** |
| S8 | `case-s8-fact-finding` | 事实认定 | S7完成 | — | light 否（整包确认）；full **是** |
| S9 | `case-s9-judgment-predict` | 要件归入与裁判预测 | S8完成 | — | light 否（整包确认）；full **是** |
| S10 | `case-s10-hallucination` | 法条幻觉校验+八个一致+FINAL阻断门禁 | S1-S9完成 | — | S10守门员阻断（light/full 均自动阻断） |

### 双视图（full 档；light 档单视图）
...
- **适用**：新案默认 light；续办未声明「升级 / 全链 / 双视图 / 开庭前」走 light；进行中 full 旧案保持 full。
- **三门停**：A4 确认 → S1-S10 连续跑完整包确认 → 对外文书生成前确认。九步包未确认不得生成起诉状/答辩状；S10 阻断不得进 FINAL。
- **单视图**：只跑代理这一侧九步目录；S3/S6 仍读对方起诉状或答辩状做抗辩和争点，不另建对侧全套九步。
- **直跑**：不自动派师傅，检索/文书/质检按需借调 SuitAgent；不切模型不换会话。
- **Git**：九步包结束后一次提交。
- **增量重跑**：律师对九步包某步提修改后，只重跑该步及其受影响下游，不整链重来。
...
| 版本 | 日期 | 更新内容 |
|------|------|----------|
| v1.1.0 | 2026-09-16 | ADR-0005 默认轻量路：三门停、单视图、总控直跑、不切会话；逐步确认与双视图降为 full 档 |

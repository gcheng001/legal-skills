# 专人派发协议（Specialist Dispatch Protocol）v1.1
...
**硬闸规则（full 档生效；light 档按 ADR-0005 休眠，总控直跑，见文末「与 ADR-0005 的关系」）**：
...
4. 本闸不约束：核对表、状态同步、LOG.md、CLAUDE.md、git 提交等机器活。light 档整闸休眠，总控直跑。
...
ADR-0005 默认轻量路下本协议**休眠不废除**：不派师傅、不硬闸、不逐步确认；检索/文书/质检可按需借调 SuitAgent 师傅，但不自动派。律师说「升级 / 全链 / 双视图 / 开庭前」切 full 档后，本协议全部规则（派发边界、硬闸、质检双闸、prompt 四段结构）原样恢复生效。

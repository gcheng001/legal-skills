...
从S1/S2/S3/S4/S5/S7/S9中提取所有法条引用，**并连带检索关联司法解释**（不只查主条）：

| 步骤 | 引用法条 | 连带检索的关联解释 | 原文 |
|------|----------|----------|------|
| S2 | 《民法典》第577条 | 买卖合同司法解释相关条款 | "依据《民法典》第577条..." |
| S3 | 《民法典》第525条 | 合同编通则解释（同时履行抗辩） | "根据《民法典》第525条..." |
| S2 | 《民法典》第1191条 | **侵权责任编解释(一)第15/17条** | 追偿权案件必查关联解释 |

> ⚠️ **关联解释铁律**（肖永吉案实证）：引主条必须连带查关联司法解释，漏引会导致论证路径错误。肖永吉案引1191条漏侵权解释(一)第15条，致"非劳动关系→不适用1191条"的论证被削弱（第15条扩张到"执行工作任务的其他人员"）。漏引记入 `statute_accuracy.missing_related_interpretations`，HIGH 风险预警（律师确认是否影响论证）。
...
### 第二步B：类案真实性校验（新增 · 肖永吉案实证）

**说明**：S9 预测若引用类案，必须核验案号真实性与连号指纹。AI 生成类案常编造连号案号，不拦则上庭穿帮。

#### 检查项

1. **案号反查**：每个引用案号反查元典案例库
   - `mcp__yuandian-case__yuandian_rh_ptal_search`（按 ah 案号精确查）
   - 或 `mcp__yuandian-case__yuandian_case_vector_search`（语义检索同类案，对比案号）
   - 查到 → `case_number_exists=true` ｜ 查不到 → `case_number_exists=false` → **CRITICAL 阻断**

2. **连号指纹检测**：案号尾号命中以下模式 → `serial_pattern_detected=true` → **CRITICAL 阻断**
   - `12345` / `123456`（顺序连号）
   - `67890` / `678901`
   - `23456` / `34567` / `45678` / `56789`
   - `11111` / `99999` / `00000`（重复数字）
   - 其他明显人为编排模式

3. **真实性存疑预警**：案号格式异常、法院代号与年份不符 → HIGH 风险（律师确认）

#### 肖永吉案实证

case-os v10.0 自动生成的深度分析报告引用 6 个类案（(2021)京03民终12345号、(2022)沪02民终67890号 等），全部命中连号指纹，元典库反查不存在。主人手工写的代理词/答辩状类案（(2024)浙0324民初181号 等）则全部真实。**印证：核验纪律主要针对自动产物。**

#### 输出

填入 `hallucination_check.case_consistency[]`，每条带 `case_number_exists` / `serial_pattern_detected` / `risk_level`。schema 语义约束8/9 会强制：连号或案号不存在 → `case_hallucination_blocked=true` 且 `can_enter_final=false`。
...
- ❌ **发现幻觉自动阻断**
- ❌ **类案连号指纹检出 / 案号反查不存在 → CRITICAL 自动阻断**（肖永吉案实证，schema 语义约束8/9）
- ❌ 不放行进入 FINAL
- ✅ 修正后必须重新校验
